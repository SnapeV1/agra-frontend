...
export class PostsService {
  ...
  private postsSubject = new BehaviorSubject<PostViewModel[]>([]);
  ...
  createPost(newPost: PostViewModel): void {
    this.posts.unshift(newPost);
    this.sortPosts();
  }

  editPost(updatedPost: PostViewModel): void {
  ...
  addComment(post: PostViewModel, comment: PostComment): void {
  ...
  private normalizePost(raw: any): PostViewModel {
  ...
  createPostOnServer(formData: FormData): Observable<PostViewModel> {
    const token = this.authService.getToken();
